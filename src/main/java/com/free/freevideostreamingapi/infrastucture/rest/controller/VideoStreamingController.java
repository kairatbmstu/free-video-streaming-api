...
public class VideoStreamingController {
    ...
    public ResponseEntity getVideoMediam3u8File(@PathVariable String id) throws IOException {
        Optional<VideoFileEntity> videoOptional = videoFileRepository.findById(id);
        if (!videoOptional.isPresent()) {
            throw new ObjectNotFound();
        }

        VideoFileEntity videoFile = videoOptional.get();
        byte[] bytes = FileUtils.readFileToByteArray(new File(videoFile.getOutputDir()+"/media.m3u8"));
    ...
    public void getVideoStreamingM3u8(@PathVariable String id, @PathVariable String filename, HttpServletResponse httpServletResponse) throws IOException {
        ...
        VideoFileEntity videoFile = videoOptional.get();
        byte[] bytes = FileUtils.readFileToByteArray(new File(videoFile.getOutputDir()+"/"+filename));
